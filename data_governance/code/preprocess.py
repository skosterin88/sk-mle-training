...
# @click.command()
# @click.argument('input_frame_path', type=click.Path(path_type=Path))
# @click.argument('output_frame_path', type=click.Path(path_type=Path))
# @click.argument('column', type=str, default='Review')
def cli_preprocess(input_frame_path: Path, 
...
if __name__ == "__main__":
    ...
    output_frame_path = os.getcwd() + sys.argv[2]

    print(input_frame_path)

    # cwd = os.getcwd()

    # input_frame_path = os.getcwd() + '/data_governance/data/input/train.csv'
    # output_frame_path = os.getcwd() + '/data_governance/data/processed/train.parquet'

    cli_preprocess(input_frame_path, output_frame_path)